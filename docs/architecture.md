...
├── index.html              # Assembled single-page portfolio (generated from src/html/ via build.ps1/sh)
├── build.ps1               # Root orchestrator: builds HTML partials & JS bundle
├── build.sh                # Bash equivalent root orchestrator
...
│   ├── html/               # HTML modular partials (<300 LOC each) & template
│   │   ├── index.template.html # HTML skeleton with <!-- @include --> directives
│   │   ├── build.ps1       # Compiles partials into root index.html
│   │   ├── build.sh        # Bash equivalent compiler
│   │   └── partials/       # Decomposed section partials
...
│   └── bundle/             # Source modules for bundle.js (<300 LOC each)
│       ├── build.ps1       # Compiles bundle.js
│       ├── build.sh        # Bash equivalent
│       ├── controllers/    # UI & interaction controllers
│       ├── i18n-service.js # Internationalization engine
│       ├── locales-en.js   # English translations
│       ├── locales-vi.js   # Vietnamese translations
│       ├── main.js         # Bootstrap dispatcher
│       ├── models.js       # Data models
│       └── utils.js        # Core DOM helpers & tokens
│
└── docs/
...
8 projects total — 4 personal, 4 work. All `.project-card` share a uniform card style with:
...
- **Inconsistency**: Personal projects (`project1`, `project_pingmap`, `project_storage`, `project2`) have rich `.project-meta` panels with 3 key-value rows + optional `.project-tech` tags. Work projects (`project3`–`project6`) have only a description and flow diagram.
...
|---|---|---|
| **Controllers** | `src/bundle/controllers/` | Handle DOM events, UI tabs, pipeline simulation, easter eggs |
| **Models & Locales** | `src/bundle/` (`models.js`, `locales-*.js`) | Terminal lines, pipeline presets, multilingual dictionary |
| **Services** | `src/bundle/i18n-service.js` | Localization detection & translation caching |
| **Utils** | `src/bundle/utils.js` | DOM helpers (`$`, `$$`), design tokens, tab switcher |
...
- **Full Site Build (HTML + JS)**:
  Run root build orchestrator:
  ```powershell
...
- **`index.html` (root)**: Assembled from `src/html/index.template.html` and `src/html/partials/`. Run:
  ```powershell
  powershell.exe -ExecutionPolicy Bypass -File src/html/build.ps1
  ```
  Or on Linux: `bash src/html/build.sh`

- **`src/bundle.js`**: Regenerated from `src/bundle/` modules. Run:
  ```powershell
  powershell.exe -ExecutionPolicy Bypass -File src/bundle/build.ps1
  ```
  Or on Linux: `bash src/bundle/build.sh`

- **`404.html`** (root): Generated stub. Source is `src/404.html`. Rebuild by copying source to root.

### Offline Compatibility

Both `index.html` and `src/bundle.js` are authored and compiled to guarantee **100% offline standalone operation**:
- Works immediately when opening `file:///path/to/index.html` via double-click without needing a local HTTP server.
- Zero runtime external network requests, zero CDNs, zero fetch calls.
- Self-hosted variable fonts (`Cascadia Code`, `Cascadia Mono`) under `fonts/`.
- Inline SVGs for all icons, eliminating external SVG spritemaps or webfont dependencies.
- No CORS restrictions under `file://` protocol.
